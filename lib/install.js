  ...
  function go () {
    ...
    mkdir(devDir, function (err, created) {
      ...
      if (process.platform === 'os390') {
        var sp;
        if (tarPath) {
          sp = spawn('pax', ['-s#node-v[0-9]*.[0-9]*.[0-9]*/##p', '-rzf', tarPath], { cwd: devDir })
        } else {
          sp = spawn('cp', [ '-r', path.dirname(process.execPath) + '/../include', '.' ], { cwd: devDir })
        }
        sp.on('error', cb)
        sp.on('close', afterTarball)
        return
      }
      ...
      function afterTarball () {
        ...
        if (extractCount === 0 && process.platform !== 'os390') {
